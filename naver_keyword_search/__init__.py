...
def platform_info():
    lines = []
    lines.append('RMM: %s' % __version__)
...
def setup_logging(appname=None, appvers=None, debug=None, filename=None,
    ...
    if filename is None:
        ...
        hand = EnhancedRotatingFileHandler(filename=filename,when='M',interval=interval,maxBytes=max_bytes,backupCount=backup_count)
...
def int_to_kor_str(value):
    """
    원단위 값 텍스트 표현
    """
    value_txt = str(value)
    if value_txt[0:1] == '-':
        number = int(value_txt[1:])
        sign = '-'
    else:
        number = int(value_txt)
        sign = ''

    # FIXME: 숫자자릿수가 8 이상인 경우 처리
    if len(str(number)) <= 4:
        number_txt = format(number)
    elif len(str(number)) <= 8:
        front_len = len(str(number)) - 4
        number_front = int(str(number)[0:front_len])
        number_back = int(str(number)[front_len:])
        number_txt = format(number_front) + '만'
        if number_back != 0:
            number_txt = number_txt + format(number_back)
    else:
        number_txt = format(number)
    return sign + number_txt
...
def send_email(text):
    ...
    from_addr = 'yungyuchoi@thinkpool.com'
    to_addr_list = [
        'mykim5179@thinkpool.com', 'ilcoffee@thinkpool.com',
        'gr2019065@thinkpool.com', 'rascu@thinkpool.com']

    server = smtplib.SMTP('smtp.hiworks.com:587')
    server.starttls()
    server.login(from_addr, 'rlawjdgh12!')

    body = MIMEMultipart()
    body['subject'] = text
    body['From'] = from_addr
    body['To'] = ",".join(to_addr_list)
